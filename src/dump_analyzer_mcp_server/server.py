...
def _build_session_id(session_id: str) -> str:
...
def get_or_create_uploaded_session(
    *,
    ...
    return upload_sessions.get_or_create_cdb_session(
        _build_session_id(analysis_session_id),
        lambda: CDBSession(
            dump_path=dump_path,
            cdb_path=cdb_path,
            symbols_path=_resolve_symbols_path(symbols_path),
            timeout=timeout,
            verbose=verbose,
        ),
    )
    ...
    async def call_tool(name, arguments: dict) -> list[TextContent]:
        try:
            ...
            if name == "start_analysis_session":
                ...
                get_or_create_uploaded_session(
            ...
            if name == "execute_windbg_command":
                ...
                try:
                    session = get_or_create_uploaded_session(
                        analysis_session_id=args.session_id,
                        dump_path=metadata.temp_file_path,
                        cdb_path=cdb_path,
                        symbols_path=symbols_path,
                        timeout=timeout,
                    ...
                    await _send_tool_progress(0, f"执行命令: {args.command}", "queued")
                    ...
                    def on_heartbeat() -> None:
                        if ctx is None:
                            return
                        fut = asyncio.run_coroutine_threadsafe(
                            _send_progress(ctx.session, request_id, None, "正在处理中...", "running"),
                            loop,
                        )
                        try:
                            fut.result(timeout=5)
                    ...
                    await _send_tool_progress(100, "命令执行完成", "completed")
                    payload = {
                        "success": True,
                        "command": args.command,
