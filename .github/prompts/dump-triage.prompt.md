...
- Ask user to provide the specific dump source.
- If the dump is already on the server host, use `list_windbg_dumps` to help find it.
- If the dump is only on the client machine, use `create_upload_session`, upload the bytes to `upload_url`, then continue with `session_id`.
