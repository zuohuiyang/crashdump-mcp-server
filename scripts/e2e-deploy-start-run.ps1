...
function Stop-StaleE2EProcesses {
    $stale = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
        $_.CommandLine -match "dump_analyzer_mcp_server --host|pytest src/dump_analyzer_mcp_server/tests/e2e"
    }
    if (-not $stale) {
...
try {
    ...
    $defaultSymbolHeavyDumpPath = Join-Path $repoRoot "tests\e2e\assets\electron.dmp"
    $testBaseUrl = ""
    ...
    Wait-PortReady -targetHost "127.0.0.1" -portNumber $port -timeoutSec 60 -process $serverProcess
