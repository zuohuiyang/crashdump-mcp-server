...
def load_e2e_config(repo_root: Path) -> E2EConfig:
    default_dump_path = repo_root / "src" / "dump_analyzer_mcp_server" / "tests" / "dumps" / "DemoCrash1.exe.7088.dmp"
    default_symbol_heavy_dump_path = repo_root / "tests" / "e2e" / "assets" / "electron.dmp"
    dump_path = Path(os.getenv("DUMP_E2E_DUMP_PATH", str(default_dump_path))).expanduser().resolve()
    base_url = os.getenv("DUMP_E2E_BASE_URL", "http://127.0.0.1:8000").rstrip("/")
