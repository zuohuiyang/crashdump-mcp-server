...
- `DUMP_E2E_MCP_TRACE`：是否打印 MCP 收发日志（默认 `1`）
- `DUMP_E2E_MCP_TRACE_MAX_CHARS`：单条 MCP 日志最大字符数（默认 `4000`，脚本中会提升）
...
- MCP 收发日志：pytest 输出中的 `[e2e-mcp][request|response_*|http_error]`（同时落盘到 pytest 日志）
- 失败时会额外输出：pytest 尾部、server 尾部、端口/进程快照、防火墙手动放行命令模板（不自动修改系统规则）

当命令执行失败时，优先查看 `[e2e-mcp]` 日志中的 `payload`、`response` 和 `raw body`，即可直接看到发送给 MCP 的参数与 MCP 返回的错误详情。
